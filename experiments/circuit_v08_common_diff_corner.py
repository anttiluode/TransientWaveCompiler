"""Fresh preregistered qualification for TW-1A v0.8 common/difference reverse."""
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        fab_ok = bool(
            fab["all_monotonic"]
            and fab["all_site_scales_positive"]
            and fab["all_headroom"]
        )
        fabrication_pass = fabrication_pass and fab_ok
        print(
            f"seed={seed} mono={fab['all_monotonic']} "
            f"positive={fab['all_site_scales_positive']} "
    ...
    summary = {
        ...
        "fabricated_tiles_pass": sum(
            bool(
                r["fabrication"]["all_monotonic"]
                and r["fabrication"]["all_site_scales_positive"]
                and r["fabrication"]["all_headroom"]
            )
            for r in rows
        ),
    ...
    Path("circuit-v08-common-diff-corner.json").write_text(
        json.dumps(
            {
                "experiment": "tw1a-v08-common-diff-formal-gate",
                "preregistration": "docs/CIRCUIT_V08_COMMON_DIFF_PREREG.md",
                "seeds": SEEDS,
