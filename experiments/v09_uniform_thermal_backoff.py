"""Partitioned-RNG uniform kT/C backoff on spent task/fabrication 2400."""
...
GRID = [0.0, 2.5e-6, 5.0e-6, 7.5e-6, 1.0e-5, 1.25e-5, 1.5e-5, 1.75e-5, 2.0e-5]


def configure(tile, b: float, dynamic_seed: int, role: int) -> None:
    # Construct formal silicon first, then zero only switch-residual fields.
    # This is the same anti-redraw rule used by the partitioned factorial.
    scale_switch_residuals(tile, 0.0)
    tile.config = replace(
        tile.config,
        edge_ktc_base_fraction=float(b),
        self_ktc_base_fraction=float(b),
        drift_ktc_base_fraction=float(b),
    )
    tile.reseed_dynamic_streams(int(dynamic_seed) * 16 + int(role))


def run_once(task, cfg, gain: float, b: float, dynamic_seed: int) -> dict:
    et, ed, st, sd = make_four(task, cfg, gain)
    for role, tile in enumerate((et, ed, st, sd)):
        configure(tile, b, dynamic_seed, role)
...
def main() -> None:
    ...
    ap.add_argument("--b", type=float, choices=GRID, required=True)
    a = ap.parse_args()
    b = float(a.b)
    ...
    rows = []
    print(f"task={TASK_SEED} fab={FABRICATION_SEED} b={b:.8g} PGA={gain:g}", flush=True)
    for dseed in DYNAMIC_SEEDS:
        row = run_once(task, cfg, gain, b, dseed)
        rows.append(row)
        print(
    ...
    tag = f"{b:.8g}".replace(".", "p").replace("-", "m").replace("+", "")
    out = {
        "experiment": "v09-partitioned-uniform-thermal-backoff",
    ...
    Path(f"v09-uniform-thermal-b-{tag}.json").write_text(
