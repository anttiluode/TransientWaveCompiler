"""Execute preregistered v0.7 50% simultaneous-corner confirmation."""
...
def main():
    ...
    result=run_point(SEEDS,cfg,'v0.7 frozen 50% simultaneous corner',final=True)
    out={
        'experiment':'tw1a_hardware_envelope_order_v07',
        'prereg':'docs/HARDWARE_ENVELOPE_ORDER_PREREG_V07.md',
        'seeds':list(SEEDS),
        'corner':CORNER,
        'result':result,
        'final_corner_earned':bool(result['summary']['qualified']),
    }
    path=Path('runs/hardware_envelope_order_v07.json')
